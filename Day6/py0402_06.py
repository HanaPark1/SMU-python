...
def score_cal(i, score):
    while True:
        score[i] = input(f"{title[i+2]} 성적을 입력하세요: ")
        if score[i].isdigit():
            score[i] = int(score[i])
            if 0 <= score[i] <= 100:
                break
...
def stu_input(count):
    while True:
        ...
        score = [0]*3 #list 생성
        for i in range(3):
            score_cal(i, score) #국어 점수 입력, 확인  
            
        total = score[0] + score[1] + score[2]
        avg = total / 3
        rank = 0
        students.append({'no': no, 'name': name, 'kor': score[0], 'eng': score[1], 
                        'math': score[2], 'total': total, 'avg': avg, 'rank': rank})
